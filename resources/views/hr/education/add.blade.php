...
<div class="card card-custom">
  ...
  <form class="form general-form form--label-right" method="POST" action="{{ route('save-education') }}" >
  @csrf
    <input type="hidden" name="id" value="{{ $detail->id ?? '' }}">
    <div class="card-body">
    
      <div class="form-group row ">
          <label class="col-xl-3 col-lg-3 col-form-label">* Türü</label>
          <div class="col-lg-6">
          ...
          <div class="col-lg-6">
            <select name="type" id="" class="select2-standard form-control">
              ...
              <option value="Etik"
                ...
                @endisset
                >Etik</option>
              <option value="İhtiyari"
                @isset($detail->type)
                  @if($detail->type == "İhtiyari")
                    selected
                  @endif
                @endisset
                >İhtiyari</option>
              <option value="Kişisel Gelişim"
                @isset($detail->type)
                  @if($detail->type == "Kişisel Gelişim")
                    selected
                  @endif
                @endisset
                >Kişisel Gelişim</option>
              <option value="Mesleki Eğitim"
                @isset($detail->type)
                  @if($detail->type == "Mesleki Eğitim")
                    selected
                  @endif
                @endisset
                >Mesleki Eğitim</option>
              <option value="Lisan"
                @isset($detail->type)
                  @if($detail->type == "Lisan")
                    selected
                  @endif
                @endisset
                >Lisan</option>
        ...
        <div class="form-group row ">
          ...
          <div class="col-lg-6">
            <select name="department_id[]" id="" class="select2-standard form-control" multiple>
              <option value="">Seçiniz</option>
              @foreach($departments as $us)
                <option value="{{ $us->id }}"
                @isset($detail)
                  @if(in_array($us->id, json_decode($selected_departments, true )))
                    selected
                  @endif
                @endisset
                >{{ $us->name }}</option>
        ...
        <div class="form-group row ">
          ...
          <div class="col-lg-6">
            <select name="user_id[]" id="" class="select2-standard form-control" multiple>
              <option value="">Seçiniz</option>
              @foreach($users as $us)
                <option value="{{ $us->id }}"
                @isset($detail)
                  @if(in_array($us->id, json_decode($selected_users, true )))
                    selected
                  @endif
                @endisset
        ...
        <div class="form-group row">
          <label class="col-xl-3 col-lg-3 col-form-label">* Eğitim Adı</label>
          <div class="col-lg-6">
            <input type="text" name="name" class="form-control" value="{{ $detail->name ?? '' }}">
          </div>
        </div>
        <div class="form-group row">
          <label class="col-xl-3 col-lg-3 col-form-label">(KGK Eğitimleri İçin) Kredi</label>
          <div class="col-lg-6">
            <input type="number" name="point" class="form-control" value="{{ $detail->point ?? '' }}">
          </div>
        </div>

        <div class="form-group row">
          <label class="col-xl-3 col-lg-3 col-form-label">* Eğitim Yeri</label>
          <div class="col-lg-6">
            <div class="typeahead">
              <input type="text" name="address" class="form-control get-places" value="{{ $detail->address ?? '' }}">
            </div>
          </div>
        </div>

        <div class="form-group row">
          <label class="col-xl-3 col-lg-3 col-form-label">* Eğitim Veren</label>
          <div class="col-lg-6">
            <input type="text" name="foundation" class="form-control" value="{{ $detail->foundation ?? '' }}">
          </div>
        </div>
        ...
        <div class="form-group row">
          ...
          <div class="col-lg-6">
            <div class="custom-file" style="margin-bottom:15px;">
                <input type="file" name="file" accept=".zip, .pdf, .pptx, .docx, .xlsx, .xls, .ppt" class="custom-file-input" id="customFile">
                <label class="custom-file-label" style="text-align:left !important;" for="customFile">Dosya seçiniz</label>
            </div>
            @isset($detail->file)
              <a href="{{ Storage::url('uploads/announcement') }}/{{ $detail->file }}" target="_blank" class="btn btn-dark"><i class="fa fa-eye"></i> Dosyayı Görüntüle</a>
            @endisset
        ...
        <div class="form-group row">
          <label class="col-xl-3 col-lg-3 col-form-label">* Eğitim Programı</label>
          <div class="col-lg-6">
        ...
        <div class="form-group row">
          <label class="col-xl-3 col-lg-3 col-form-label">* Eğitim Tarihi</label>
          <div class="col-lg-3">
...
<script>
  $(document).ready(function(){
  const firms = new Bloodhound({
    datumTokenizer: datum => Bloodhound.tokenizers.whitespace(datum.value),
    queryTokenizer: Bloodhound.tokenizers.whitespace,
      remote: {
          url: "{{ route('education-places') }}",
  ...
  $('.get-places').typeahead(null, {
    name: 'countries',
    source: firms.ttAdapter()
  });
