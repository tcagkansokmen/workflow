    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            ...
            <div class="card-toolbar">
              <!--begin::Button-->
              @if($authenticated->power('product', 'add'))
              <a href="{{ route('add-product') }}" class="btn btn-primary font-weight-bolder">
        ...
        <div class="card-body">
            <div class="row mb-15">
                <div class="col-sm-3">
                    ...
                    'class' => 'select2-standard category-filter',
                    ])
                    @endcomponent
                </div>
            </div>
...
@section('scripts')
    ...
    var costtable = $('#productcategories').DataTable({
        ...
        "deferRender": true,
        ajax: {
            url: "{{ route('products-json') }}?{!! \Request::getQueryString() !!}",
        },
        ...
        "drawCallback": function( settings ) {
            ...
            $('body').on('change', '.category-filter', function(e){
                var val = $(this).val();
                costtable.column(3)
                .search(val)
                .draw();
        ...
        columns: [
        { data: 'title', name: 'title', title: 'Ürün', "render": function (data, type, row) {
        ...
        { data: 'category.title', name: 'category.title', title: 'Kategori', "render": function (data, type, row) {
        ...
        { data: 'id', className:"align-right", title: 'İşlemler', name: 'id', "render": function(data, type, row) {
            var editme = '<a href="{{ route("add-product") }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-theme="light" title="Düzenle">\
                <i class="flaticon2-writing icon-md text-primary"></i>\
                </a>&nbsp;';
            var detailme = '<a href="{{ route("product-detail", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-warning btn-sm" data-toggle="tooltip" data-theme="light" title="Detay"><i class="fas fa-chart-line icon-md text-warning"></i></a>&nbsp;';

            detailme = '';

            var deleteme = "";

              deleteme = '<a href="{{ route("delete-product", ["id" => "."]) }}/'+data+'" class="btn btn-icon btn-light btn-hover-danger btn-sm delete-cost" data-toggle="tooltip" data-theme="light" title="Sil">\
    ...
    { data: 'category_id', className:"align-right", visible: false }
    ],
    buttons: [
    {
        extend: 'print',
