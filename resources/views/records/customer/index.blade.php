    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            ...
            <div class="card-toolbar">
              <!--begin::Button-->
              @if($authenticated->power('customers', 'add'))
              <a href="{{ route('add-customer') }}" class="btn btn-primary font-weight-bolder">
...
@section('scripts')
    ...
    var costtable = $('#productcategories').DataTable({
        ...
        "deferRender": true,
        ajax: {
            url: "{{ route('customers-json') }}?{!! \Request::getQueryString() !!}",
        },
        ...
        columns: [
        { data: 'title', name: 'title', title: 'Müşteri', "render": function (data, type, row) {
        return '<div class="d-flex align-items-center" style="max-width:300px;">\
                        ...
                        ' + row.personel.name + ' ' + row.personel.surname + '\
                        </a>\
                        <div>\
                    </div>\
                    </div>\
        ...
        { data: 'projects_count', name: 'projects_count', title: 'Proje', searchable: false, "render": function (data, type, row) {
                return '<span class="label label-lg font-weight-bold  label-light-info label-inline label-light-danger label-inline" >'+ data +'</span>';
            }
        },
        { data: 'id', className:"align-right", title: 'İşlemler', name: 'id', "render": function(data, type, row) {
            var editme = '<a href="{{ route("add-customer") }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-theme="light" title="Düzenle">\
                <i class="flaticon2-writing icon-md text-primary"></i>\
                </a>&nbsp;';
            var detailme = '<a href="{{ route("customer-detail") }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-warning btn-sm" data-toggle="tooltip" data-theme="light" title="Detay"><i class="fas fa-chart-line icon-md text-warning"></i></a>&nbsp;';
            var deleteme = "";

            if (row.projects_count<1){
              deleteme = '<a href="{{ route("delete-customer", ["id" => "."]) }}/'+data+'" class="btn btn-icon btn-light btn-hover-danger btn-sm delete-cost" data-toggle="tooltip" data-theme="light" title="Sil">\
