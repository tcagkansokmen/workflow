    ...
    @if (config('layout.extras.notifications.display'))
        ...
        @else
            <div class="dropdown">
                {{-- Toggle --}}
                <div class="topbar-item" data-toggle="dropdown" data-offset="10px,0px">
                    <div class="btn btn-icon btn-clean btn-dropdown btn-lg mr-1 {{ $authenticated->notifications() ? 'pulse pulse-danger' : '' }}">
                        {{ Metronic::getSVG("media/svg/icons/General/Notifications1.svg", "svg-icon-xl svg-icon-".($authenticated->notifications() ? 'danger' : 'primary')) }}
                        <span class="pulse-ring"></span>
                        @if($authenticated->notifications())
                        {{ $authenticated->readNotification('/'.Request::path()) }}
                            <span class="notification-count label label-sm label-rounded label-light-danger">{{ $authenticated->notifications() }}</span>
                        @endif
                    </div>
                </div>
        ...
        @if (config('layout.extras.user.layout') == 'offcanvas')
            <div class="topbar-item">
                <div class="btn btn-icon w-auto btn-clean d-flex align-items-center btn-lg px-2" id="kt_quick_user_toggle">
                    ...
                    <span class="text-dark-50 font-weight-bolder font-size-base d-none d-md-inline mr-3">{{$authenticated->name}} {{$authenticated->surname}}</span>
