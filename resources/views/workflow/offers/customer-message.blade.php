{{-- Extends layout --}} @extends('layout.empty') {{-- Content --}} @section('content')
            ...
            <div class="card-body p-0">
              ...
              @foreach($messages as $d)
                <div class="cursor-pointer shadow-xs {{ $loop->first ? 'toggle-on' : 'toggle-off' }}" data-inbox="message">
                  ...
                  @isset($d->user)
                    {{ $d->user->userAvatar(50, 'info', 'mr-4') ?? '' }}
                  @else 
                    <span class="symbol symbol-50 mr-4">
                      <span class="symbol-label font-size-h5 font-weight-bold">{{ strtoupper(mb_substr($d->from, 0, 1)) }}</span>
                    </span>
                  @endisset
                    ...
                    <input type="hidden" name="offer_id" value="{{ $detail->id }}">
                      ...
                      <div class="d-block">
                        ...
                        <div class="d-flex align-items-center border-bottom inbox-to px-8 min-h-50px">
                          <div class="text-dark-50 w-75px">Gönderen:</div>
                          <div class="d-flex align-items-center flex-grow-1">
                            <input type="text" class="form-control border-0" name="from" />
                          </div>
                        </div>
                        @endisset
                        <!--end::To-->
                        ...
                        <div class="d-flex align-items-center border-bottom inbox-to px-8 min-h-50px">
                          ...
                          <div class="d-flex align-items-center flex-grow-1">
                            <input type="text" class="form-control border-0" name="compose_to" value="{{ isset(Auth::user()->email) ? '' : $detail->user->email }}," />
                          </div>
                          <div class="ml-2">
