	...
	<div class="container">
		...
		<div class="d-flex flex-row">
			...
			<div class="flex-row-auto offcanvas-mobile w-300px w-xl-350px" id="kt_profile_aside">
				...
				<div class="card card-custom gutter-b">
					...
					<div class="card-body pt-4">
							...
							<div>
								...
								<div class="mt-2">
									@if(Auth::user()->isAdmin()) 
										@if($detail->contract_status=='Yönetici Onayında')
										<a href="{{ route('contract-status', ['id' => $detail->id]) }}" data-status="Yönetici Onayladı" class="btn btn-sm btn-success offer-status font-weight-bold mr-2 py-2 px-3 px-xxl-5 my-1">Onay</a>
										<a href="{{ route('contract-status', ['id' => $detail->id]) }}" data-status="Yönetici Reddetti" class="btn btn-sm btn-danger offer-status font-weight-bold py-2 px-3 px-xxl-5 my-1">Red</a>
										@endif 
									@endif
									@if(Auth::user()->id==$detail->user_id && $detail->contract_status=='Hazırlanıyor')
									<a href="{{ route('contract-status', ['id' => $detail->id]) }}" data-status="Yönetici Onayında" class="btn btn-sm btn-success offer-status font-weight-bold mr-2 py-2 px-3 px-xxl-5 my-1">Onaya
										Gönder
									</a>
									@elseif($detail->contract_status=='Yönetici Onayladı')
									<a href="{{ route('contract-message', ['id' => $detail->id]) }}" class="btn btn-sm btn-success font-weight-bold mr-2 py-2 px-3 px-xxl-5 my-1">Müşteriye
										Gönder
									</a>
									@elseif($detail->contract_status=='Müşteri Onayında')
									<a href="{{ route('contract-status', ['id' => $detail->id]) }}" data-status="müşteri onayladı" class="btn btn-sm btn-success offer-status font-weight-bold mr-2 py-2 px-3 px-xxl-5 my-1">Müşteri
										Onayladı
									</a>
									<a href="{{ route('contract-status', ['id' => $detail->id]) }}" data-status="müşteri reddetti" class="btn btn-sm btn-danger offer-status font-weight-bold py-2 px-3 px-xxl-5 my-1">Müşteri
										Reddetti
									</a>
									@endif 
								</div>
						...
						<!--begin::Contact-->
						<!--end::Contact-->
						@if(strpos($detail->contract_status, 'onayında') !== false)
						<span href="#" class="btn btn-light-warning font-weight-bold py-3 px-6 mb-2 text-center btn-block">{{ $detail->contract_status }}
						</span>
						@elseif(strpos($detail->contract_status, 'onay') !== false)
						<span href="#" class="btn btn-light-success font-weight-bold py-3 px-6 mb-2 text-center btn-block">{{ $detail->contract_status }}
						</span>
						@elseif(strpos($detail->contract_status, 'red') !== false)
						<span href="#" class="btn btn-light-danger font-weight-bold py-3 px-6 mb-2 text-center btn-block">{{ $detail->contract_status }}
						</span>
						@else
						<span href="#" class="btn btn-light-primary font-weight-bold py-3 px-6 mb-2 text-center btn-block">{{ $detail->contract_status }}
						</span>
						@endif
					</div>
					<!--end::Body-->
				...
				<div class="card card-custom mb-5">
					<div class="card-body py-4">
						<div class="d-flex">
							@if($detail->contract)
								<a href="{{ Storage::url('snap/contract/') }}{{ $detail->contract }}" target="_blank" class="btn btn-bold btn-lg btn-light-primary btn-block mr-5 mt-0">
									Sözleşme
								</a>
							@endif
									<a href="{{ route('offer-detail', ['id' => $detail->id]) }}" class="btn btn-bold btn-lg btn-light-info btn-block mt-0">Teklif</a>
						</div>
					</div>
				</div>
				...
				<div class="card card-custom">
					...
					<div class="card-header h-auto py-4">
						<div class="card-title">
							<h3 class="card-label">Genel Sözleşme Bilgileri
				...
				<div class="card card-custom mt-10">
					...
					<div class="card-header h-auto py-4">
						<div class="card-title">
							<h3 class="card-label">Ödeme Vadeleri
								<span class="d-block text-muted pt-2 font-size-sm">brief'e ait şirket ve Proje bilgileri</span></h3>
						</div>
					</div>
					<!--end::Header-->
					...
					<div class="card-body py-4">
						@foreach($bills as $b)
						<div class="form-group row my-2">
							<label class="col-4 col-form-label">{{ number_format($b['price'], 2, ",", ".") }}</label>
							<div class="col-8">
								<span class="form-control-plaintext font-weight-bolder">{{ date('d-m-Y', strtotime($b['bill_date'])) }}</span>
							</div>
						</div>
						@endforeach
					</div>
			...
			<div class="flex-row-fluid ml-lg-8">
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="card card-custom card-stretch gutter-b">
							...
							<div class="card-header h-auto py-4">
								<div class="card-title">
									<h3 class="card-label">Sözleşme Dosyaları
				...
				<div class="card card-custom mt-5">
					...
					<div class="card-header align-items-center px-4 py-3">
						<div class="text-center flex-grow-1">
							<div class="text-dark-75 font-weight-bold font-size-h5">Sözleşmeye Ait Yorumlar</div>
							<div>
					...
					<div class="card-body">
						...
						<div data-scroll="true" style="max-height:500px;">
							...
							<div class="messages offer-comment">
								@include('workflow.contracts.contract-comment')
							</div>
							<!--end::Messages-->
						</div>
						<!--end::Scroll-->
					...
					<div class="card-footer align-items-center">
						<form action="{{ route('save-contract-comment') }}" method="POST" class="offer-comment-form">
							@csrf
				...
				<a href="{{ route('contract-message', ['id' => $detail->id]) }}" class="btn lg-sm btn-success btn-lg btn-block font-weight-bold mt-5 mr-2 py-2 px-3 px-xxl-5 my-1">Müşteri Mesajları
				</a>
