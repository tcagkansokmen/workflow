...
<div class="alert alert-custom alert-white alert-shadow gutter-b" role="alert">
	<div class="alert-text">
        <form action="{{ route('briefs') }}" method="get">
            <div class="row align-items-end" >
                <div class="col-sm-9">
                    <div class="row">
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                            <label style="display:block;">Tasarımcı</label>
                            @component('components.forms.select', [
                                'required' => false,
                                'name' => 'designer_id',
                                'value' => Request::query('designer_id') ? Request::query('designer_id') : '',
                                'values' => $designers,
                                'class' => 'select2-standard'
                                ])
                            @endcomponent
                ...
                <div class="col-sm-3">
                    <div class="form-group">
                        ...
                        <a href="{{ route('briefs') }}" class="btn btn-primary">Sıfırla</a>&nbsp;
                    </div>
                </div>
            </div>
        </form>
    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            ...
            @if(Auth::user()->department_id!=2)
                <!--begin::Button-->
                <a href="{{ route('add-brief') }}" class="btn btn-primary font-weight-bolder">
        ...
        <div class="card-body">
            <div class="tablo">
              <table class="table table-striped- table-hover table-checkable" id="brief-listesi">
              </table>
            </div>
...
@section('scripts')
    ...
    $("body").on('click', '.brief-sil', function(e){
        e.preventDefault();
        var thi = $(this);
    ...
    var qrform = $('#brief-listesi').DataTable({
        responsive: true,
        "processing": true,
        "serverSide": true,
        "ajax": "{{ route('briefs-json') }}?{!! \Request::getQueryString() !!}",
        ...
        columns: [
            ...
            { data: 'customer.title', name: 'customer.title', title: 'Müşteri/Proje', "render": function (data, type, row) {
                return '<div class="d-flex align-items-center" style="max-width:220px;">\
                        ...
                        <div class="text-dark-75 font-weight-bolder font-size-lg mb-0">\
                            ...
                            <a href="{{ route("brief-detail", ["id" => "."]) }}/'+row.id+'" class="text-muted font-weight-bold text-hover-primary">' + row.project.title + "</a>\
                        <div>\
                    </div>\
                    </div>\
                </div>"
            ...
            { data: 'id', name: 'id', title: 'İşlem', "render": function(data, type, row) {
                var messageme = '<a href="{{ route("brief-detail", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-danger btn-sm" data-toggle="tooltip" data-theme="light" title="'+row.messages+' Yeni Mesaj"><i class="fas fa-envelope icon-md text-danger"></i></a>&nbsp;';

                var editme = '<a href="{{ route("update-brief", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-theme="light" title="Düzenle">\
                <i class="flaticon2-writing icon-md text-primary"></i>\
                </a>&nbsp;';
                var detailme = '<a href="{{ route("brief-detail", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-warning btn-sm" data-toggle="tooltip" data-theme="light" title="Detay"><i class="fas fa-chart-line icon-md text-warning"></i></a>&nbsp;';

                var deleteme = "";

                deleteme = '<a href="{{ route("delete-brief", ["id" => "."]) }}/'+data+'" class="btn btn-icon btn-light btn-hover-danger btn-sm delete-cost" data-toggle="tooltip" data-theme="light" title="Sil">\
