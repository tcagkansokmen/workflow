    ...
    <div class="card-body pt-4">
      ...
      <div class="d-flex align-items-center justify-content-between mb-2">
        <div style="text-align:center; width:45%;">
          ...
          <span class="btn btn-sm btn-block btn-light-success">{{ $detail->permissionsLeft() }} Gün</span>
        </div>
        <div style="text-align:center; width:45%;">
          <span class="font-weight-bold mr-2">Avans Durumu</span>
          @if($detail->earnestLeft()>0)
          <span class="btn btn-sm btn-block btn-light-info">{{ money_formatter($detail->earnestLeft()) }} TL</span>
          @else
          <span class="btn btn-sm btn-block btn-light-danger">{{ money_formatter($detail->earnestLeft()) }} TL</span>
          @endif
        </div>
      </div>
      <!--end::Contact-->
      ...
      <div class="navi navi-bold navi-hover navi-active navi-link-rounded mt-10">
        <div class="navi-item mb-2">
          <a href="{{ route('personel-detail', ['id' => $detail->id]) }}" class="navi-link py-4 {{ (request()->is('users/profil')) ? 'active' : '' }}">
        ...
        <div class="navi-item mb-2">
          <a href="{{ route('personel-earnest', ['id' => $detail->id]) }}" class="navi-link py-4 {{ (request()->is('users/earnest')) ? 'active' : '' }}">
            <span class="navi-icon mr-2">
              <span class="svg-icon">
        ...
        <div class="navi-item mb-2">
          <a href="{{ route('personel-permission', ['id' => $detail->id]) }}" class="navi-link py-4 {{ (request()->is('users/permission')) ? 'active' : '' }}">
            <span class="navi-icon mr-2">
              <span class="svg-icon">
        ...
        <div class="navi-item mb-2">
          <a href="{{ route('personel-belonging', ['id' => $detail->id]) }}" class="navi-link py-4 {{ (request()->is('users/belonging')) ? 'active' : '' }}">
            <span class="navi-icon mr-2">
              <span class="svg-icon">
