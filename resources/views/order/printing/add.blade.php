...
@section('content')
    ...
    <div class="card card-custom">
        ...
        <!--begin::Form-->
        <div class="card-body">
            <form action="{{route('save-printing')}}" class="general-form" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                  <div class="col-sm-12">
                    <div class="row">
                        ...
                        <div class="col-sm-8">
                            <div class="form-group">
                            <label>Baskı Operatörü Seçin</label>
                            @component('components.forms.select', [
                                'required' => true,
                                'name' => 'printer_id',
                                'value' => $detail->printer_id ?? '',
                                'values' => $users,
                            ...
                            <div class="dropzone-item-static dz-processing " style="">
                              <div class="dropzone-file">
                                <div class="dropzone-filename" title="some_image_file_name.jpg">
                                  ...
                                  <strong>(
                                  <span><strong><a href="{{ Storage::url('snap/printing/') }}{{ $b->filename }}" target="_blank"><i class="flaticon-download"></i> Görüntüle</a></strong></span>)</strong>
                                </div>
                              ...
                              <div class="dropzone-toolbar">
                                <a href="{{ route('delete-printing-extra', ['id' => $b->id]) }}" class="dropzone-delete delete-extra" >
                                  <i class="flaticon2-cross"></i>
                                  </a>
                              </div>
                            </div>
                    ...
                    <hr>
                    <h5 class="mt-5 mb-5">Detaylar</h5>
                    <div class="row">
                    @foreach($metas as $m)
                    <div class="col-sm-4 mb-4">
                        ...
                        @include('components.forms.input', [
                            ...
                            'name' => 'meta['.$m->key.']',
                            'class' => '',
                            'value' => isset($detail_meta)&&isset($detail_meta[$m->key]) ? $detail_meta[$m->key]["value"] : null
                        ])
                        @else 
                        <div class="form-group">
                        <label style="display:block;">{{ $m->value }}</label>
                        <select
                            class="form-control m-input select2-standard"
                            name="meta[{{ $m->key }}]">
                            <option value="">Seçiniz</option>
                            @foreach($m->options as $o)
                                <option value="{{ $o->value }}"
                                @if(isset($detail_meta)&&isset($detail_meta[$m->key])&&$detail_meta[$m->key]["value"]==$o->value)
                                    selected
                                @endif
                                >{{ $o->value }}</option>
                            @endforeach
