    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            ...
            <div class="card-toolbar">
              <!--begin::Button-->
              @if($authenticated->power('printing', 'add'))
              <a href="{{ route('add-printing') }}" class="btn btn-primary font-weight-bolder">
...
@section('scripts')
    ...
    var costtable = $('#productcategories').DataTable({
        ...
        "deferRender": true,
        ajax: {
            url: "{{ route('printings-json') }}?{!! \Request::getQueryString() !!}",
        },
        ...
        { data: 'customer.title', name: 'customer.title', title: 'Müşteri/Proje', "render": function (data, type, row) {
                return '<div class="d-flex align-items-center" style="max-width:220px;">\
                        ...
                        <a href="{{ route("printing-detail", ["id" => "."]) }}/'+row.id+'" class="text-muted font-weight-bold text-hover-primary">\
                        ' + row.project.title + '\
                        </a>\
                        <div>\
                    </div>\
        ...
        { data: 'status', name: 'status', title: 'Durum', "render": function (data, type, row) {
            ...
            if(data=='Baskı Başladı'){
                ...
                return '<span class="label label-lg font-weight-bold  label-light-warning label-inline label-light-info label-inline" >Kabul Edildi</span>';
            }
          }
        },
        { data: 'id', className:"align-right", title: 'İşlemler', name: 'id', "render": function(data, type, row) {
            var statusme = '<div class="dropdown new-dropdown dropdown-inline">\
                            ...
                            <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right" style="width:275px;">\
                                ...
                                <li class="navi-item change-status '+ (row.status=='Talep Açıldı' ? '' : 'd-none') +' " onclick="change_status('+data+', \'Kabul Edildi\')" data-id="'+data+'">\
                                    <a href="#" class="navi-link">\
                                    <span class="navi-icon">\
                                        <i class="flaticon2-shopping-cart-1"></i>\
                                    </span>\
                                    <span class="navi-text new-status">Talep Açıldı</span>\
                                    </a>\
                                </li>\
                                <li class="navi-item '+ (row.status=='Kabul Edildi' ? '' : 'd-none') +'">\
                                    <a href="#" class="navi-link change-status" onclick="change_status('+data+', \'Baskı Başladı\')" data-id="'+data+'">\
                                    <span class="navi-icon">\
                                        <i class="flaticon2-calendar-8"></i>\
                                    </span>\
                                    <span class="navi-text new-status">Baskı Başladı</span>\
                                    </a>\
                                </li>\
                                <li class="navi-item '+ (row.status=='Baskı Başladı' ? '' : 'd-none') +'">\
                                    <a href="#" class="navi-link change-status" onclick="change_status('+data+', \'Baskı Tamamlandı\')" data-id="'+data+'">\
                                    <span class="navi-icon">\
                                        <i class="flaticon2-checkmark"></i>\
                                    </span>\
                                    <span class="navi-text new-status">Baskı Tamamlandı</span>\
                                    </a>\
                                </li>\
                                </ul>\
                            </div>\
                        </div>'
            var editme = '<a href="{{ route("add-printing") }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-theme="light" title="Düzenle">\
                <i class="flaticon2-writing icon-md text-primary"></i>\
                </a>&nbsp;';
            var detailme = '<a href="{{ route("printing-detail", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-warning btn-sm" data-toggle="tooltip" data-theme="light" title="Detay"><i class="fas fa-chart-line icon-md text-warning"></i></a>&nbsp;';

            var deleteme = "";

            deleteme = '<a href="{{ route("delete-printing", ["id" => "."]) }}/'+data+'" class="btn btn-icon btn-light btn-hover-danger btn-sm delete-cost" data-toggle="tooltip" data-theme="light" title="Sil">\
            <i class="flaticon2-trash icon-md text-danger"></i>\
            </a>';

            var messageme = '<a href="{{ route("printing-detail", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-danger btn-sm" data-toggle="tooltip" data-theme="light" title="'+row.messages+' Yeni Mesaj"><i class="fas fa-envelope icon-md text-danger"></i></a>&nbsp;';
