			...
			<div class="flex-row-auto offcanvas-mobile w-300px w-xl-350px" id="kt_profile_aside">
						...
						<div class="d-flex align-items-center">
							<div class="symbol symbol-60 symbol-xxl-100 mr-5 align-self-start align-self-xxl-center">
								<div class="symbol symbol-light-info mr-3">
									<span class="symbol-label font-size-h5">Ü</span>
								</div>
							</div>
							<div>
								...
								@if($authenticated->power('production', 'status'))
									<a href="#" class="btn btn-sm btn-light-primary change-status" 
									onclick="change_status('{{ $detail->id }}', 'Başlandı')" 
									data-id="{{ $detail->id }}"><i class="flaticon2-calendar-8"></i> Başlandı</a>&nbsp;
									<a href="#" class="btn btn-sm btn-light-success change-status" 
									onclick="change_status('{{ $detail->id }}', 'Tamamlandı')" 
									data-id="{{ $detail->id }}"><i class="flaticon2-checkmark"></i> Tamamlandı</a>&nbsp;
								@endif
								</div>
							</div>
						</div>
				...
				<div class="card card-custom mb-5">
					<div class="card-body py-4">
						<div class="d-flex">
              @if($authenticated->power('production', 'edit'))
            		<a href="{{ route('update-production', ['id' => $detail->id]) }}" class="btn btn-lg btn-light-success font-weight-bolder mr-3">Düzenle</a>
							@endif
						</div>
					</div>
				</div>
				...
				<div class="card card-custom gutter-b">
					...
					<div class="card-header h-auto py-4">
						<div class="card-title">
							<h3 class="card-label">Genel Üretim Bilgileri</h3>
						</div>
					</div>
					<!--end::Header-->
					<!--begin::Body-->
						...
						<div class="card card-custom card-stretch gutter-b">
							...
							<div class="card-body">
								<div class="showgallery" data-scroll="true" data-height="475">
									@foreach($photos as $b)
									<!--begin::Item-->
									<a href="{{ Storage::url('snap/production/') }}{{ $b->filename }}" class="active-gallery d-flex align-items-center flex-wrap mb-10">
										<!--begin::Symbol-->
										<div class="symbol symbol-50 symbol-light mr-5">
											<span class="symbol-label">
														{!! image_format(Storage::url('snap/production/').$b->filename) !!}
											</span>
										</div>
										<!--end::Symbol-->
										<!--begin::Text-->
        ...
        @endif
				...
				<div class="card card-custom">
					...
					<div class="card-body">
						...
						<div data-scroll="true" style="max-height:500px;">
							...
							<div class="messages offer-comment">
								@include('order.production.messages')
							</div>
							<!--end::Messages-->
						</div>
						<!--end::Scroll-->
					...
					<div class="card-footer align-items-center">
						<form action="{{ route('save-production-message') }}" method="POST" class="comment-form">
							@csrf
...
var dropzone_design_comment = new Dropzone(design_comment_id, { // Make the whole body a dropzone
    url: "{{ route('upload-production') }}", // Set the url for your upload script location
    parallelUploads: 20,
