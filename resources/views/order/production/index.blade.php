    ...
    <div class="card card-custom gutter-b">
        <div class="card-header flex-wrap py-3">
            ...
            <div class="card-toolbar">
              <!--begin::Button-->
              @if($authenticated->power('production', 'add'))
              <a href="{{ route('add-production') }}" class="btn btn-primary font-weight-bolder">
...
@section('scripts')
    ...
    var costtable = $('#productcategories').DataTable({
        ...
        "deferRender": true,
        ajax: {
            url: "{{ route('productions-json') }}?{!! \Request::getQueryString() !!}",
        },
        ...
        { data: 'id', className:"align-right", title: 'İşlemler', name: 'id', "render": function(data, type, row) {
            var statusme = '<div class="dropdown new-dropdown dropdown-inline">\
                            ...
                            <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right" style="width:275px;">\
                                ...
                                <li class="navi-item">\
                                    <a href="#" class="navi-link change-status" onclick="change_status('+data+', \'Başlandı\')" data-id="'+data+'">\
                                    <span class="navi-icon">\
                                        <i class="flaticon2-calendar-8"></i>\
                                    </span>\
                                    <span class="navi-text new-status">Başlandı</span>\
                                    </a>\
                                </li>\
                                <li class="navi-item">\
                                    <a href="#" class="navi-link change-status" onclick="change_status('+data+', \'Tamamlandı\')" data-id="'+data+'">\
                                    <span class="navi-icon">\
                                        <i class="flaticon2-checkmark"></i>\
                                    </span>\
                                    <span class="navi-text new-status">Tamamlandı</span>\
                                    </a>\
                                </li>\
                                </ul>\
                            </div>\
                        </div>'
            var editme = '<a href="{{ route("add-production") }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-theme="light" title="Düzenle">\
                <i class="flaticon2-writing icon-md text-primary"></i>\
                </a>&nbsp;';
            var detailme = '<a href="{{ route("production-detail", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-warning btn-sm" data-toggle="tooltip" data-theme="light" title="Detay"><i class="fas fa-chart-line icon-md text-warning"></i></a>&nbsp;';

            var deleteme = "";

            deleteme = '<a href="{{ route("delete-production", ["id" => "."]) }}/'+data+'" class="btn btn-icon btn-light btn-hover-danger btn-sm delete-cost" data-toggle="tooltip" data-theme="light" title="Sil">\
            <i class="flaticon2-trash icon-md text-danger"></i>\
            </a>';

            var messageme = '<a href="{{ route("production-detail", ["id" => "."]) }}/'+data+'" class="btn btn btn-icon btn-light btn-hover-danger btn-sm" data-toggle="tooltip" data-theme="light" title="'+row.messages+' Yeni Mesaj"><i class="fas fa-envelope icon-md text-danger"></i></a>&nbsp;';
...
function change_status(id, val){
    ...
    $.ajax({
        url: '{{ route("update-production-status") }}',
        dataType: 'json',
        type: 'post',
        data: 'id=' + id + '&val=' + val,
        errur: function(){
