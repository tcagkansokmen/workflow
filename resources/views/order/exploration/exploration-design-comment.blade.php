...
<div class="d-flex flex-column mb-5 align-items-start">
  ...
  <div class="mt-1 showgallery">
    @foreach($c->files as $f)
      @if(pathinfo(Storage::url('snap/exploration/').$f->file, PATHINFO_EXTENSION)=='jpg'||pathinfo(Storage::url('snap/exploration/').$f->file, PATHINFO_EXTENSION)=='jpeg' || pathinfo(Storage::url('snap/exploration/').$f->file, PATHINFO_EXTENSION)=='png'||pathinfo(Storage::url('snap/exploration/').$f->file, PATHINFO_EXTENSION)=='gif')
        <a href="{{ Storage::url('snap/exploration/') }}{{ $f->file }}" class="active-gallery">
          <img src="{{ Storage::url('snap/exploration/') }}{{ $f->file }}" style="border-radius:5px; background:#f7f7f7; width:120px; height:120px; object-fit:cover; margin:3px;" alt="">
        </a>
      @else 
        <a href="{{ Storage::url('snap/exploration/') }}{{ $f->file }}" class="active-gallery">
